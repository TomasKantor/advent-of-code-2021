...
with open('input.txt') as file:
    ...
    for num in numbers:
        winning_boards = cross_value_in_boards(num, boards)

        if len(boards) == 1 and len(winning_boards) > 0:
            answer = get_board_sum(winning_boards[0]) * num
            print(answer)

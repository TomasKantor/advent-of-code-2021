...
# values at array next to given array are incremented by 1
# risk levels at value more than 9 reset to 1
def get_array_value(array, x, y):
    max_risk_level = 9
    height = len(array)
    width = len(array[0])

    increment_x, x = divmod(x, width)
    increment_y, y = divmod(y, height)

    increment = increment_x + increment_y

    # risk levels at value more than 9 reset to 1
    value = array[y][x] + increment - 1
...
def path_total_cost(array, path,):
    ...
    for place in path[1:]:
        p_x = place[0]
        p_y = place[1]
        total += get_array_value(array, p_x, p_y)
    ...
    while len(open_set) > 0:
        ...
        for n in neighbors:
            ...
            if not check_valid_coords(n_x, n_y, width, height):
                continue
            tentative_g_score = g_score.get(current, math.inf) + get_array_value(array, n_x, n_y)
